...
class _LoanApprovalPageState extends State<LoanApprovalPage> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow('Application Date', _formatDate(data['applicationDate'])),
            _buildInfoRow('Disbursement Date', _formatDate(data['disbursementDate'])),
            _buildInfoRow('Due Date', _formatDate(data['dueDate'])),
            _buildInfoRow('Interest Rate', '${data['interestRate']}%'),
            _buildInfoRow('Monthly Payment', currencyFormat.format(data['monthlyPayment'])),
            _buildInfoRow('Total Repayment', currencyFormat.format(data['totalRepayment'])),
            _buildInfoRow('Remaining Balance', currencyFormat.format(data['remainingBalance'])),
          ],
        ),
      ),
    );
  ...
  Future<void> _submitDecision() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Loan ${_decision!} successfully')),
      );
